...
# Silme butonu ayrı olarak ekleniyor
butonListesi.append(Button((700, 470), 320, 80, butonListeDegerleri[4][0]))
...
while True:
    ...
    if hands:
        ...
        length, _, img = detector.findDistance(lmList[12][:2], lmList[8][:2], img)
        print(length)
        x, y = lmList[12][:2]
        if length < 70:
            for i, buton in enumerate(butonListesi):
                if buton.klikKontrol(x, y) and gecikmeSayaci == 0:
                    mevcutDeger = buton.deger
                    ...
                    if mevcutDeger == '=':
                        ...
                        denklemim = denklemim[:-1]  # Remove last character
                    else:
                        denklemim += mevcutDeger
                    gecikmeSayaci = 1  #

    # Çoklu Göstermeleri engelleme
    if gecikmeSayaci != 0:
        gecikmeSayaci += 1
        if gecikmeSayaci > 10:
            gecikmeSayaci = 0
